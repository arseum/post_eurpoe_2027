...
function getChapter(w) {
    return w <= 10 ? 1 : w <= 20 ? 2 : 3;
}

function getArmyCap() {
    return 5 + (state.buildings.includes('quartiers') ? 3 : 0);
}
...
function findEvent() {
    return EVENTS.find(e => !state.eventsSeen.includes(e.id) && e.wave === state.wave && (!e.requires || e.requires(state)));
}
